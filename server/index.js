...
// Open Playground endpoint for file analysis (correct routing)
app.post('/api/open-playground', async (req, res) => {
  try {
    const { message, code, selectedFileIds } = req.body;
    ...
    // Create session for open playground
    const sessionKey = crypto.randomBytes(16).toString('hex');
    ...
    sessions.set(sessionKey, {
      ...
      code: analysisCode,
      filename: combinedFilename,
      message: message || `Analyze ${selectedFileIds?.length || 1} contract file(s) for security vulnerabilities`,
      createdAt: new Date().toISOString(),
      selectedFileIds: selectedFileIds || []
    });

    console.log('🎯 Open playground session created:', {
      sessionKey,
      userId: decodedUser.userId,
      hasCode: !!analysisCode,
      codeLength: analysisCode?.length || 0,
    ...
    res.json({
      success: true,
      sessionKey,
      message: 'Session created for open playground analysis'
    });

  } catch (error) {
    console.error('❌ Open playground endpoint error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to create open playground session'
    });
  }
});
